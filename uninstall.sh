...
if ! systemctl enable mbpfan.service || ! systemctl restart mbpfan.service; then
  systemctl start omarchy-mac-fan-max.service 2>/dev/null || true
  echo "Could not restore mbpfan; system helper was left installed for recovery." >&2
  exit 1
fi
...
rm -f /var/lib/omarchy-mac-fan-control/mbpfan.conf.original
rmdir /var/lib/omarchy-mac-fan-control 2>/dev/null || true
